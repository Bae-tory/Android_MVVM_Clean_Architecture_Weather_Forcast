...
open class BaseRecyclerViewAdapter : RecyclerView.Adapter<BaseViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder {
        return object : BaseViewHolder(parent, viewType) {
            override fun onBindViewHolder(item: Any?, vm: Any?, eventHolder: Any?) {
                super.onBindViewHolder(item, vm, eventHolder)
            }
        }
    }

    override fun getItemCount(): Int =
        items.count() + headerCount/*0 or 1*/ + footerCount/*0 or 1*/
    ...
    private fun isHeaderPosition(position: Int) =
        headerCount != 0 && position == 0

    private fun isFooterPosition(position: Int) =
        footerCount != 0 && position == itemCount - 1
